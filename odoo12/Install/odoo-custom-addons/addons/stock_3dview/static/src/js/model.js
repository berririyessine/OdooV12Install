    ...
    var ThreeDModel = AbstractModel.extend({
        ...
        load: async function (params) {
            ...
            */
            ...
            // we must wait for this to complete before fetching the actual data...
        ...
        _loadLegend: function (params) {
            let request_type = modelData.action.customdata_request_type || 'tagged';
        ...
        /**
         * Sets the action that called the 3D view.
         * 
         * Returns a Promise, because we must wait until the Event Dispatcher does its job.
         *
         * @param {Object} params
         * @returns {Deferred}
         */
        _setAction: async function() {
            console.log("waiting for execution of _setAction");
            return new Promise(function(resolve, reject) {
                let interval = setInterval(function() {
                    console.log("executing _setAction");
                    var obj = modelData.parent.actions;
                    if (obj) {
                        for (let key in obj) {
                            if (obj[key]['view_mode'].includes('threedview')) {
                                console.log("found one");
                                modelData.action = obj[key];
                                console.log(modelData.action);
