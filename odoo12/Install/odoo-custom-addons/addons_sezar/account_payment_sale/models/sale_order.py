...
class SaleOrder(models.Model):
    ...
    def _action_confirm(self):
        ...
        for rec in self.order_line:
            if rec.product_uom_qty > rec.total_qty_product:
                verif=False
                raise ValidationError ("Quantité Insuffisante" + " " + "(" + str(rec.name) + ")")
        if verif==True :
            # raise ValidationError (str(verif))
    ...
    def set_to_draft(self):
        ...
        stock_picking = self.env['stock.picking'].search([('origin','=',self.name),('name','like','%OUT%')])[-1]
