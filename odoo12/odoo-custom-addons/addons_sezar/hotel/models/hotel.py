...
class HotelFloor(models.Model):
    ...
    name = fields.Char("Nom de l'étage", required=True, index=True)
...
class HotelRoomType(models.Model):
    ...
    categ_id = fields.Many2one("hotel.room.type", "Catégorie")
    child_ids = fields.One2many(
        "hotel.room.type", "categ_id", "Sous Catégories"
    )
...
class ProductProduct(models.Model):
    ...
    isroom = fields.Boolean("Est une pièce")
    iscategid = fields.Boolean("Est une catégorie")
    isservice = fields.Boolean("Est un Service")
...
class HotelRoomAmenitiesType(models.Model):
    ...
    name = fields.Char(required=True)
    amenity_id = fields.Many2one("hotel.room.amenities.type", "Catégorie")
    child_ids = fields.One2many(
        "hotel.room.amenities.type", "amenity_id", "Sous catégorie"
    )
...
class HotelRoomAmenities(models.Model):
    ...
    categ_id = fields.Many2one(
        "hotel.room.amenities.type", string=" Catégorie Équipements ", required=True
    )
    product_manager = fields.Many2one("res.users", string="Responsable Produits")
...
class HotelRoom(models.Model):
    ...
    floor_id = fields.Many2one(
        "hotel.floor", "Numéro d'étage", help="À quel étage se trouve la chambre."
    )
    max_adult = fields.Integer()
    max_child = fields.Integer()
    categ_id = fields.Many2one(
        "hotel.room.type", string="Categorie Chambre", required=True
    )
    room_amenities = fields.Many2many(
        ...
        "room_amenities",
        "rcateg_id",
        help="Liste des équipements des chambres. ",
    )
    ...
    capacity = fields.Integer("Capacité", required=True)
    room_line_ids = fields.One2many(
        "folio.room.line", "room_id", string="Ligne de réservation de chambre"
    )
    product_manager = fields.Many2one("res.users", "Responsable Produits")

    @api.constrains("capacity")
    def check_capacity(self):
        for room in self:
            if room.capacity <= 0:
                raise ValidationError(_("La capacité de la chambre doit être supérieure à 0"))
...
class HotelFolio(models.Model):
    ...
    name = fields.Char(
        "N° Reservation", readonly=True, index=True, default="Nouveau"
    )
    order_id = fields.Many2one(
        "sale.order", "Commande", delegate=True, required=True, ondelete="cascade"
    )
    ...
    room_lines = fields.One2many(
        ...
        help="Détails de la réservation de la chambre d'hôtel.",
    )
    service_lines = fields.One2many(
        ...
        readonly=True,
        states={"draft": [("readonly", False)], "sent": [("readonly", False)]},
        help="Détails des services hôteliers fournis à"
        "Client et il sera inclus dans "
    ...
    hotel_policy = fields.Selection(
        ...
        default="manual",
        help="Politique de l'hôtel pour le paiement"
 "soit le client doit payer à "
 "heure de réservation ou d'enregistrement"
 "heure de départ.",
    )
    duration = fields.Float(
        "Durée en jours",
        help="Nombre de jours qui seront automatiquement "
 "compter à partir de la date d'arrivée et de départ.",
    )
    hotel_invoice_id = fields.Many2one(
        "account.invoice", "Facture", copy=False
    )
    duration_dummy = fields.Float("Durée Estimée")
    ...
    def folio_room_lines(self):
        ...
        for room in self[0].room_lines:
            if room.product_id.id in folio_rooms:
                raise ValidationError(_("Vous ne pouvez pas prendre la même chambre deux fois"))
...
class HotelFolioLine(models.Model):
    ...
    order_line_id = fields.Many2one(
        "sale.order.line",
        string="Ligne des commandes",
        required=True,
        delegate=True,
        ondelete="cascade",
    )
    folio_id = fields.Many2one(
        "hotel.folio", string="Reservation", ondelete="cascade"
    )
    checkin_date = fields.Datetime(
        string="Date d'arrivée", required=True, default=_get_checkin_date
    )
    checkout_date = fields.Datetime(
        string="Date de départ", required=True, default=_get_checkout_date
    )
    is_reserved = fields.Boolean(
        string="Réservé",
        help="Vrai lorsque la ligne Reservation est créée à partir de \
 Réservation",
    )
...
class HotelServiceLine(models.Model):
    ...
    ser_checkin_date = fields.Datetime(
        "À partir de la date", required=True, default=_service_checkin_date
    )
    ser_checkout_date = fields.Datetime(
        "À ce jour", required=True, default=_service_checkout_date
    )
...
class HotelServiceType(models.Model):
    ...
    name = fields.Char("Nom de Service", size=64, required=True)
    service_id = fields.Many2one("hotel.service.type", " Categorie de Service")
    child_ids = fields.One2many(
        "hotel.service.type", "service_id", "Sous Categorie"
    )
...
class HotelServices(models.Model):
    ...
    categ_id = fields.Many2one(
        "hotel.service.type", string=" Categorie de Service", required=True
    )
    product_manager = fields.Many2one("res.users", string="Responsable Produits")
...
class ResCompany(models.Model):
    ...
    additional_hours = fields.Integer(
        ...
        help="Fournissez la valeur minimale d'heures pour \
 jours d'arrivée, de départ, quel que soit le \
 les heures seront fournies ici en fonction de \
 que les jours supplémentaires seront calculés.",
    )
